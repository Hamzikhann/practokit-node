		...
		if (error) {
			...
			if (!oldQuizAttributes) {
				quizResponse.forEach((element) => {
					...
					timeSpend =
						element.remainingDuration >= 0 ? timeSpend + (element.duration - element.remainingDuration) : timeSpend;
				});
				...
				quizResponse.forEach((element, index) => {
					if (element.questionType.title == "Fill in the Blank") {
						...
						if (
							...
							list[crypto.decrypt(element.id)] == crypto.decrypt(element.selectedOption)
						) {
							result += element.points;
							quizResponse[index].isWrong = false;
						} else {
				...
				QuizSubmissions.create(
					...
					.then(async (submissionRes) => {
						await QuizSubmissionResponse.create(
							{ response: JSON.stringify(quizResponse), quizSubmissionId: submissionRes.id },
							{ transaction }
						);
	...
	try {
		...
		Quizzes.findAll({
			...
			.catch((err) => {
				emails.errorEmail(req, err);
				res.status(500).send({
					message: err.message || "Some error occurred while retrieving Submission of user by quiz and user Id."
				});
			});
	} catch (err) {
		emails.errorEmail(req, err);
